...
#  NCErrorMit.py
# --------------------
# Implements a QML nearest centroid classifier and supports simple error mitigation
...
def classify(sample):
    ...
    for c in CLASSES:
        ...
        for i in range(3): # Error mitigation #1
            p += estimateDistance(sample, centroids[c])  
        p/=3
        EPS = 1e-10  # Error mitigation #2
        p1 = float(np.clip(p[1], EPS, 1.0 - EPS))
        cosXY = np.sqrt(p1)                           
