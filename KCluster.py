...
#  KCluster.py
# --------------------
# Implements a QML nearest centroid classifier utilizing K-Means clustering
...
for label in CLASSES:
    ...
    km.fit(filtered_data[train_indices[label]])
    for center in km.cluster_centers_:
        centroids.append(center)
        centroid_norms.append(np.linalg.norm(center))
...
def classify(sample):
    ...
    for c in range(len(centroids)):
        ...
        dists[c] = dist
    returnable = min(dists, key=dists.get)
    return cluster_labels[returnable]
...
for index in testIndex:
    ...
    if correctAnswer == pred:
        correct += 1
print(f"\nAccuracy: {correct} / {len(testIndex)} = {correct / len(testIndex):.2%}")
print("K-Cluster")
